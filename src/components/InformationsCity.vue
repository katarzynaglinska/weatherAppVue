            ...
            :class="[
              'menu-prediction__item',
              {
                'menu-prediction__item--selected':
                  selectedPrediction === tab + 'Prediction'
              }
            ]"
          >
            {{ tab }}
...
export default {
  ...
  methods: {
    ...
    buildList: function(res) {
      ...
      this.dataCityForecast = res.forecast;
      {
        this.dataCityHistory.map(
          (object, i) => (this.pm10History[i] = object.values[2].value)
        );
      }
      {
        this.dataCityHistory.map(
          (object, i) => (this.pm25History[i] = object.values[1].value)
        );
      }
      {
        this.dataCityHistory.map(
      ...
      {
        this.dataCityHistory.map(
          (object, i) => (this.temperatureHistory[i] = object.values[5].value)
        );
      }
      {
        this.dataCityHistory.map(
          (object, i) =>
            (this.datasHistory[i] = moment
              .utc(this.dataCityHistory[i].fromDateTime)
              .format("DD/MM HH:mm"))
        );
      }
      {
        this.dataCityForecast.map(
          (object, i) => (this.pm10Forecast[i] = object.values[0].value)
        );
      }
      {
        this.dataCityForecast.map(
          (object, i) => (this.pm25Forecast[i] = object.values[1].value)
        );
      }
      {
        this.dataCityForecast.map(
      ...
      }
      {
        this.dataCityForecast.map(
          (object, i) =>
            (this.datasForecast[i] = moment
              .utc(this.dataCityForecast[i].fromDateTime)
              .format("DD/MM HH:mm"))
        );
      }
    ...
    setCurrentData() {
      var day = moment.utc(
        this.dataCityCurrent.fromDateTime,
        "YYYY-MM-DD HH:mm:ss"
      );
      var dayNameOfWeek =
        day
